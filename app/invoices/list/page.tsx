...
function InvoiceListPageContent() {
  ...
  const filteredInvoices = invoices.filter((inv) => {
    ...
    switch (statusFilter) {
      ...
      case "ready":
        return s === "READY_TO_VALIDATE";
      case "validated":
        return s === "VALIDATED";
      case "error":
        return ["ERROR", "REJECTED", "TO_VERIFY", "VERIFY"].includes(s);
      default:
        return true;
    }
  });
  ...
  const handleUpload = async (files: File[]) => {
    try {
      ...
      toast.success(
        `${files.length} facture${files.length > 1 ? "s" : ""} import̩e${files.length > 1 ? "s" : ""}`,
      );
      await loadData();
    } catch {
  ...
  const handleComptabiliser = async (id: number) => {
    try {
      ...
      setInvoices((prev) =>
        prev.map((inv) => (inv.id === id ? { ...inv, accounted: true } : inv)),
      );
      toast.success("Facture comptabilis̩e");
    } catch (err: any) {
      const details = err?.details?.details || err?.details;
      const apiError =
        err?.details?.error || err?.details?.message || err?.message;
      const msg = details
  ...
  const counts = {
    ...
    pending: invoices.filter((i) =>
      ["PENDING", "PROCESSING"].includes((i.status || "").toUpperCase()),
    ).length,
    ready: invoices.filter(
      (i) => (i.status || "").toUpperCase() === "READY_TO_VALIDATE",
    ).length,
    validated: invoices.filter(
      (i) => (i.status || "").toUpperCase() === "VALIDATED",
    ).length,
    error: invoices.filter((i) =>
      ["ERROR", "REJECTED", "TO_VERIFY", "VERIFY"].includes(
  ...
  return (
    <div className="w-full py-0 space-y-3">
