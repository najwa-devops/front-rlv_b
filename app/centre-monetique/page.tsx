...
export default function CentreMonetiquePage() {
  ...
  const loadHistory = async () => {
    try {
      const data = await api.getCentreMonetiqueBatches(100)
  ...
  useEffect(() => {
    const resetOnRefresh = async () => {
      setLoading(true)
      try {
        const existing = await api.getCentreMonetiqueBatches(200)
        if (Array.isArray(existing) && existing.length > 0) {
          await Promise.all(existing.map((batch) => api.deleteCentreMonetiqueBatch(batch.id)))
        }
      } catch (error) {
  ...
  const openDetail = async (id: number) => {
    try {
      const detail = await api.getCentreMonetiqueBatchById(id, true)
      setSelected(detail)
      setDetailOpen(true)
    } catch (error: any) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      for (const f of files) {
        const response = await api.uploadCentreMonetique(f, undefined, selectedStructure)
  ...
  const handleReprocess = async (id: number) => {
    try {
      const response = await api.reprocessCentreMonetique(id, undefined, selectedStructure)
  ...
  const handleOpenFile = async (id: number) => {
    try {
      const response = await fetch(api.getCentreMonetiqueFileUrl(id))
      if (!response.ok) {
  ...
  const handleSaveRows = async () => {
    ...
    try {
      ...
      const response = await api.saveCentreMonetiqueRows(selected.id, payload)
      setSelected(response.batch)
