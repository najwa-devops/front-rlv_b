...
export function ClientLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="flex flex-col h-screen bg-background text-foreground">
            {/* Top navigation bar */}
            <header className="h-14 border-b border-border/50 bg-card/30 backdrop-blur-md flex items-center justify-between px-3 sm:px-6 shrink-0">
                <div className="flex items-center gap-2 sm:gap-3">
                    <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-primary/20">
                        <Sparkles className="h-4 w-4 text-primary" />
                    </div>
                    <span className="font-bold text-foreground tracking-tight text-sm sm:text-base">BankingOCR</span>
                    <span className="text-xs text-muted-foreground hidden md:block">— Relevés Bancaires</span>
                </div>

                <div className="flex items-center gap-1 sm:gap-3">
                    {user && (
                        <div className="flex items-center gap-2 sm:gap-3 border-r border-border/50 pr-2 sm:pr-3 mr-1">
                            <div className="h-8 w-8 rounded-lg bg-primary/10 border border-primary/20 flex items-center justify-center text-primary font-bold text-sm shrink-0">
                                {userInitial}
                            </div>
