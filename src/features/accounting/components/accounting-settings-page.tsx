...
export function AccountingSettingsPage() {
    ...
    const [accountClassFilter, setAccountClassFilter] = useState<number | null>(null);
    const [tierFilter] = useState<string>("all");

    const [isAccountDialogOpen, setIsAccountDialogOpen] = useState(false);
    const [editingAccount, setEditingAccount] = useState<Account | null>(null);
    const [accountForm, setAccountForm] = useState({ code: "", libelle: "", tvaRate: 20, taxCode: "", active: true });
    ...
    const filteredAccounts = useMemo(() => {
        ...
        if (accountClassFilter !== null) {
            result = result.filter(a => a.classe === accountClassFilter || a.code.startsWith(String(accountClassFilter)));
        }
    ...
    function openAddAccount() {
        ...
        setAccountForm({ code: "", libelle: "", tvaRate: 20, taxCode: "", active: true });
    ...
    const chargeAccounts = accounts.filter(a => a.classe === 6 || a.code.startsWith("6"));
    const tvaAccounts = accounts.filter(a => a.code.startsWith("345") || a.code.startsWith("445"));
                ...
                <TabsContent value="accounts" className="space-y-4 pt-4">
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle className="text-xl font-semibold">Liste des Comptes</CardTitle>
                            <div className="flex items-center gap-2">
                                ...
                                <Button onClick={openAddAccount}><Plus className="h-4 w-4 mr-2" />Ajouter</Button>
                            </div>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            <div className="flex flex-wrap gap-2 mb-2">
                                <Button variant={accountClassFilter === null ? "default" : "outline"} size="sm" onClick={() => setAccountClassFilter(null)}>Tous</Button>
                                {[1, 2, 3, 4, 5, 6, 7].map(c => (
                                    <Button
                                        key={c}
                ...
                <DialogContent>
                    ...
                    <div className="grid gap-4 py-4">
                        <div className="grid grid-cols-4 items-center gap-4">
                            ...
                            <Input className="col-span-3" value={accountForm.code} disabled={!!editingAccount} onChange={e => setAccountForm({ ...accountForm, code: e.target.value })} />
                        </div>
                        <div className="grid grid-cols-4 items-center gap-4">
                            <Label className="text-right">Libelle</Label>
                            <Input className="col-span-3" value={accountForm.libelle} onChange={e => setAccountForm({ ...accountForm, libelle: e.target.value })} />
                        </div>
                    </div>
                    <DialogFooter>
                    ...
                    <div className="grid gap-4 py-4">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-2">
                                ...
                                <Input value={tierForm.libelle} onChange={e => setTierForm({ ...tierForm, libelle: e.target.value })} />
                            </div>
                            <div className="space-y-2">
                                <Label>Compte Tier (ex: 44111234)</Label>
                                <Input className="font-mono" value={tierForm.tierNumber} onChange={e => setTierForm({ ...tierForm, tierNumber: e.target.value })} />
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 border-t pt-4">
                            <div className="space-y-2">
                                ...
                                <Input value={tierForm.ifNumber} onChange={e => setTierForm({ ...tierForm, ifNumber: e.target.value })} />
                            </div>
                            <div className="space-y-2">
                                <Label>ICE</Label>
                                <Input value={tierForm.ice} onChange={e => setTierForm({ ...tierForm, ice: e.target.value })} />
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 border-t pt-4">
                            <div className="space-y-2">
                                ...
                                <Select
                                    ...
                                    onValueChange={v => setTierForm({ ...tierForm, defaultChargeAccount: v })}
                                >
                                    <SelectTrigger>
                                    ...
                                    <SelectContent>
                                        {chargeAccounts.map(a => (
                                            <SelectItem key={a.id} value={a.code}>{a.code} - {a.libelle}</SelectItem>
                                        ))}
                                    </SelectContent>
                                </Select>
                            </div>
                            <div className="space-y-2">
                                ...
                                <Select
                                    ...
                                    onValueChange={v => setTierForm({ ...tierForm, tvaAccount: v })}
                                >
                                    <SelectTrigger>
                                        <SelectValue placeholder="Choisir un compte" />
                                    </SelectTrigger>
