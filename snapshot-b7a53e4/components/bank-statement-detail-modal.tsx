...
import type { BankStatementV2, BankTransactionV2, Tier } from "@/lib/types"
...
export function BankStatementDetailModal({
    ...
    const getTierAccount = (tier: Tier): string => {
        const maybeSupplierAccount = (tier as Tier & { supplierAccount?: string }).supplierAccount
        return maybeSupplierAccount || tier.displayAccount || tier.tierNumber || ""
    }
    ...
    const renderEditableCell = (
        ...
        const value = (tx[field] ?? "") as string | number
    ...
    const handleSaveAll = async () => {
        ...
        const existingRows = toPersist.filter((tx) => tx.id > 0)
        const localRows = toPersist.filter((tx) => tx.id <= 0)

        setSaving(true)
        try {
            for (const tx of existingRows) {
                ...
                const updated = await api.updateBankTransaction(tx.id, payload)
                if (onUpdateTransaction) onUpdateTransaction(updated)
            }

            for (const tx of localRows) {
                const created = await api.createBankTransaction({
                ...
                if (onUpdateTransaction) onUpdateTransaction(created)
            }
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-8 gap-3 items-end">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) => setNewTransaction((prev) => ({ ...prev, compte: e.target.value }))}
                                    />
                                </div>
                                        ...
                                        {sortByIndex(editableTransactions).map((tx) => (
                                            ...
                                            <TableCell>
                                                {isEditingCell(tx.id, "compte") ? (
                                                    renderEditableCell(tx, "compte", { className: "font-mono", emptyLabel: "Non lié" })
                                                ) : (
                                                    <Popover open={isAccounted ? false : undefined}>
                                                            ...
                                                            <Command>
                                                                ...
                                                                <CommandGroup className="max-h-[300px] overflow-auto">
                                                                    {loadingTiers ? (
                                                                        ...
                                                                        const account = getTierAccount(tier)
                                                                        return (
                                                                            <CommandItem
                                                                                key={tier.id}
                                                                                value={`${account} ${tier.libelle}`}
                                                                                onSelect={() => {
                                                                                    setEditableTransactions((prev) =>
                                                                                        prev.map((row) =>
                                                                                            row.id === tx.id
                                                                                                ? { ...row, compte: account, isLinked: true }
                                                                                                : row
                                                                                        )
                                                                                    )
                                                                                }}
                                                                            ...
                                                                            >
                                                                                <div className="flex items-center w-full justify-between">
                                                                                    <span className="font-medium text-sm">{tier.libelle}</span>
                                                                                    {tx.compte === account && (
