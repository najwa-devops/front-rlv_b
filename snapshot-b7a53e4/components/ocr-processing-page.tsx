...
import { TierCreationModal } from "./tier-creation-modal"
...
export function OcrProcessingPage({
  ...
  }, [invoice.rawOcrText, invoice.extractedText, invoice.headerText, invoice.footerText])
  ...
  const handleLinkTier = async (selectedTierId: number) => {
    ...
    try {
      ...
      if (result.invoice) {
        ...
        toast.success(`Fournisseur "${updatedInvoice.tier?.libelle}" lié avec succès`, { id: toastId })
      }
    } catch (error: any) {
  ...
  const handleTierSelect = (selectedTier: Tier) => {
    ...
    const ifNumber = String(fields.find(f => f.key === 'ifNumber')?.value || '')
    const rcNumber = String(fields.find(f => f.key === 'rcNumber')?.value || '')
    ...
    const needsUpdate = (!selectedTier.ice && ice) ||
  ...
  const handleOcrExtract = async (forcedTemplateId?: number) => {
    ...
    try {
      ...
      if (forcedTemplateId) {
        ...
        const currentWorkflowStatus = toWorkflowStatus(status || invoice.status)
      ...
      if (extractionFields) {
        const updatedFields = ParseBackendFieldsData(extractionFields, fields, result, newText)
        setFields(updatedFields)
      ...
      const is500 = errorMessage.includes("500") || errorMessage.includes("EXTRACTION_500");
      const isMissingOcr = errorMessage.includes("Texte OCR non disponible") || errorMessage.includes("EXTRACTION_400");

      if (isMissingOcr) {
        toast.error("Données OCR manquantes", {
          description: "Ce document ne contient pas de texte numérisé exploitable. Il est peut-être mal scanné ou l'upload a échoué.",
          duration: 6000,
          action: {
            label: "Compris",
  ...
  const updateFieldValue = (key: string, value: string | number) => {
    ...
    setFields((prev) => prev.map((f) => (f.key === key ? { ...f, value: valueStr } : f)))
    ...
    const checkExistingSupplier = async () => {
      ...
      try {
        ...
        if (foundTier) {
          ...
          if (foundTier.collectifAccount) updateFieldValue("collectifAccount", foundTier.collectifAccount)
          if (foundTier.tierNumber) updateFieldValue("tierNumber", foundTier.tierNumber)
          if (foundTier.defaultChargeAccount) updateFieldValue("chargeAccount", foundTier.defaultChargeAccount)
          if (foundTier.tvaAccount) updateFieldValue("tvaAccount", foundTier.tvaAccount)
          if (foundTier.defaultTvaRate) updateFieldValue("tvaRate", foundTier.defaultTvaRate)
        } else {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!isDemoMode) {
        console.log("Sending fields and patterns to backend:", fieldsForBackend, fieldPatterns)
        ...
        if (savedInvoice.autoFilledFields) setAutoFilledFields(savedInvoice.autoFilledFields)
  ...
  const renderFieldCard = (field: DynamicInvoiceField | undefined, isCustom: boolean = false) => {
      ...
      <Card key={field.key} className="shadow-sm hover:shadow-md transition-shadow">
        <CardContent className="p-2 space-y-1.5">
            ...
            <div className="flex items-center gap-1">
              {isSelectingPosition === field.key ? (
                <Button
                  size="sm"
                  variant="ghost"
                  className={`h-7 px-2 text-[10px] animate-pulse ${selectionMode === 'PATTERN'
          ...
          <Input
            ...
            className={`bg-white transition-all ${isPending ? "border-amber-500 bg-amber-50/10" : ""
  ...
  return (
    <div className="min-h-screen bg-background p-6">
              ...
              <div className="overflow-auto rounded-lg border border-border">
                ...
                {!isLoadingDocument && !documentError && documentUrl && (
                  <div
                    ...
                    className={`relative ${isSelectingPosition && selectionMode === 'VALUE'
                      ? "cursor-crosshair"
                      : isSelectingPosition
                        ? "cursor-text"
                        : ""
          ...
          <Card>
            <CardHeader className="pb-3">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-base">Texte OCR Complet (Raw)</CardTitle>
              </div>
            </CardHeader>
            <CardContent>
              <Textarea
          ...
          {!invoice.templateDetected && !tier && !isExistingSupplier && (
            <Card className="border-indigo-600/30 bg-indigo-50/50 dark:bg-indigo-950/10 shadow-sm border-2 overflow-hidden">
              <div className="bg-indigo-600 px-4 py-1.5 flex items-center justify-between">
                ...
                <Badge variant="outline" className="text-[9px] border-indigo-400 text-indigo-100 uppercase py-0 px-1.5 font-bold">Nouveau</Badge>
              </div>
              <CardContent className="pt-4 pb-5">
                <div className="space-y-4">

                  <p className="text-sm font-medium text-slate-700 dark:text-slate-300">
                    Cette facture provient d'un nouveau fournisseur ou n'est pas encore reliée à un compte tiers.
                  </p>
                  ...
                  <div className="grid grid-cols-2 gap-3 bg-white dark:bg-slate-900/50 p-4 rounded-xl border-2 border-indigo-100 dark:border-indigo-900/30 shadow-sm">
                    <div className="space-y-1">
                      <p className="text-[10px] uppercase font-bold text-indigo-600 dark:text-indigo-400">ICE détecté 🤖</p>
                      <p className="text-[13px] font-mono font-bold text-slate-900 dark:text-slate-100">{fields.find(f => f.key === "ice")?.value || "Non détecté"}</p>
                    </div>
                    <div className="space-y-1 border-l pl-4 border-slate-100 dark:border-slate-800">
                      <p className="text-[10px] uppercase font-bold text-indigo-600 dark:text-indigo-400">Fournisseur (OCR)</p>
                      <p className="text-[13px] font-bold uppercase text-slate-900 dark:text-slate-100 truncate">{fields.find(f => f.key === "supplier")?.value || "Non détecté"}</p>
                    </div>
                  </div>
          ...
          <Card>
            ...
            <CardContent className="p-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {renderFieldCard(fields.find(f => f.key === 'invoiceDate') || fields[1])}
                {renderFieldCard(fields.find(f => f.key === 'invoiceNumber') || fields[0])}

                {/* Ligne 2: Fournisseur | Compte Tiers */}
                {renderFieldCard(fields.find(f => f.key === 'supplier') || { ...fields[2], key: 'supplier', label: 'Fournisseur' })}
                {renderAccountFieldCard('tierNumber', 'Compte Tiers', 'tierNumber')}

                {/* Ligne 3: Montant HT | Compte HT */}
                {renderFieldCard(fields.find(f => f.key === 'amountHT') || fields[3])}
                {renderAccountFieldCard('chargeAccount', 'Compte HT', 'defaultChargeAccount')}

                {/* Ligne 4: TVA | Compte TVA */}
                {renderFieldCard(fields.find(f => f.key === 'tva') || fields[4])}
                {renderAccountFieldCard('tvaAccount', 'Compte TVA', 'tvaAccount')}
              ...
              <Card>
                <CollapsibleTrigger asChild>
                  <CardHeader className="cursor-pointer hover:bg-muted/50 transition-colors pb-3 py-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">HEADER</Badge>
              ...
              <Card>
                <CollapsibleTrigger asChild>
                  <CardHeader className="cursor-pointer hover:bg-muted/50 transition-colors pb-3 py-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <Badge variant="outline" className="bg-purple-50 text-purple-700 border-purple-200">FOOTER</Badge>
                        <CardTitle className="text-sm">Texte OCR extrait</CardTitle>
                      </div>
                      <ChevronDown className="h-4 w-4 text-muted-foreground" />
                    </div>
      ...
      <TierCreationModal
        ...
        initialData={{
          libelle: String(fields.find(f => f.key === 'supplier')?.value || ''),
          ice: String(fields.find(f => f.key === 'ice')?.value || ''),
          ifNumber: String(fields.find(f => f.key === 'ifNumber')?.value || ''),
          rcNumber: String(fields.find(f => f.key === 'rcNumber')?.value || ''),
        }}
      ...
      <TierSelectionModal
        ...
        initialSearch={String(fields.find(f => f.key === 'supplier')?.value || fields.find(f => f.key === 'ice')?.value || '')}
      />
      ...
      <CreateTemplateModal
        ...
        initialSupplierName={String(fields.find(f => f.key === "supplier")?.value || "")}
        onSuccess={(templateId) => {
