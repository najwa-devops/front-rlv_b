...
export default function InvoicesPage() {
    ...
    const loadInvoices = async () => {
        try {
            ...
            const pending = localInvoices.filter(inv => toWorkflowStatus(inv.status) !== "VALIDATED")
            setInvoices(pending)
        } catch (err) {
    ...
    const applyFilters = (invoicesList: DynamicInvoice[]) => {
        return invoicesList.filter((invoice) => {
            ...
            if (filters.status && filters.status !== "all") {
                if (toWorkflowStatus(invoice.status) !== String(filters.status).toUpperCase()) return false
            }
    ...
    const handleProcessInline = async (invoice: DynamicInvoice) => {
        try {
            ...
            const workflowStatus = toWorkflowStatus(invoice.status)
            if (workflowStatus === "VERIFY") {
            ...
            toast.error(message, { id: `process-${invoice.id}` })
        }
    }
    ...
    const handleConfirmInvoice = async (invoice: DynamicInvoice) => {
        try {
            toast.loading("Confirmation en cours...", { id: `confirm-${invoice.id}` })
            await api.updateInvoiceStatus(invoice.id, "READY_TO_TREAT")
            await loadInvoices()
            toast.success(`✓ Facture confirmée par le fournisseur`, { id: `confirm-${invoice.id}` })
        } catch (err) {
    ...
    const handleFinalValidate = async (invoice: DynamicInvoice) => {
        try {
            toast.loading("Validation finale en cours...", { id: `validate-${invoice.id}` })
            await api.validateInvoice(invoice.id)
            await loadInvoices()
            toast.success(`✓ Facture validée définitivement`, { id: `validate-${invoice.id}` })
        } catch (err) {
            toast.error("Erreur de validation finale", { id: `validate-${invoice.id}` })
        }
    }
